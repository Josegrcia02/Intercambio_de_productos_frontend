...
const ProductDetailPage = () => {
  ...
  useEffect(() => {
    if (showExchangeModal && product?.id) {
       // Obtenemos las conversaciones para ver con quién ha hablado
       api.get('/mensajes/conversaciones').then(res => {
           const usuariosUnicos = [];
           const idsVistos = new Set();

           // Filtramos para obtener SOLO los chats de ESTE producto en concreto
           const chatsDeEsteProducto = res.data.filter(chat => Number(chat.producto_id) === Number(product.id));

           chatsDeEsteProducto.forEach(chat => {
  ...
  const submitExchange = async () => {
    ...
    try {
        ...
        toast.success("¡Felicidades! Intercambio registrado correctamente.");
        setShowExchangeModal(false);
        navigate('/perfil'); // Volvemos al perfil
    } catch (error) {
        console.error(error);
        toast.error("Error al registrar el intercambio.");
    }
  };
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-6xl relative">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 lg:gap-12">
        ...
        <div className="space-y-4 relative">
          ...
          {product.status === 'PENDIENTE' && (
            <div className="absolute top-4 left-4 z-10 bg-orange-500 text-white px-4 py-1.5 rounded-lg font-bold shadow-lg flex items-center gap-2">
              <Lock className="w-4 h-4" /> EN REVISIÓN
            </div>
          )}
          ...
          <div className="flex flex-col gap-3 pt-2">
            ...
            {product.status === 'PENDIENTE' ? (
              <div className="w-full space-y-3">
                 <div className="w-full bg-orange-50 border border-orange-200 text-orange-800 py-4 px-4 rounded-xl text-center font-bold text-lg">
                ...
                {isOwner ? (
                  ...
                  <>
                    ...
                    <Button 
                      ...
                      onClick={() => toast.info("Envía un mensaje al vendedor para solicitar la reserva.")}
                    >
      ...
      <ConfirmModal 
        ...
        message="¿Estás seguro de que quieres eliminar esta publicación? No podrás deshacer esta acción."
      />
    </div>
  );
};
